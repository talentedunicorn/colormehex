<template>
  <main id="app" :style="containerStyles">
    ...
    <footer>
      <p>
        Made by
        <a href="https://talentedunicorn.com"
          >Talentedunicorn</a
        >
      </p>
      <p>
...
import { computed, onMounted, ref, type ComputedRef, type StyleValue } from "vue";
...
const hexName: ComputedRef<string> = computed(() => {
    let text = name.value.trim().toLowerCase();
    text = hashCode(text).toString();
    text = intToRGB(parseInt(text));
    return `#${text}`;
  });

const invertedHex: ComputedRef<string> = computed(() => invert(hexName.value, true));

const twitterLink = () => {
  const description = "A fun toy that changes your text to colors";
  return `https://www.twitter.com/intent/tweet?url=${rootUrl}&text=${encodeURIComponent(description)}`;
};
...
const containerStyles: ComputedRef<StyleValue> = computed(() => ({
